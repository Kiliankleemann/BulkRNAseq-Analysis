...
#Create Directories
dir.create('results')
dir.create('results/featurecounts')
dir.create('plots')
...
# Loop through each file to read and extract relevant columns
for (file in file_list) {
  # Read the FeatureCounts file
  count_data <- fread(file, skip = 1)  # Skip the header line
  sample_name <- sub("\\.txt$", "", basename(file))  # Remove .txt extension for sample name
  
  # Dynamically identify the counts column
  count_col <- setdiff(names(count_data), c("Geneid", "Chr", "Start", "End", "Strand", "Length"))[1]
  
  # Check if the column was found
  if (is.null(count_col)) {
    stop(paste("Counts column not found in file:", file))
  }
  
  # Rename the Counts column to include the sample name
  setnames(count_data, count_col, sample_name)
  
  # Keep only Geneid and the renamed column
  count_list[[sample_name]] <- count_data[, .(Geneid, get(sample_name))]
  setnames(count_list[[sample_name]], "V2", sample_name)  # Rename the second column properly
}
...
#Assign rownames
merged_counts <- merged_counts %>% column_to_rownames("Geneid")

# Save the combined matrix to a file
write.table(merged_counts, "results/featurecounts/combined_featurecounts_matrix.txt", sep = "\t", row.names = FALSE, quote = FALSE)

#Clean columns for DESeq2 analysis
colClean <- function(merged_counts){colnames(merged_counts) <- gsub("_featurecounts_output", "", colnames(merged_counts)); merged_counts } 
merged_counts <- colClean(merged_counts)


# Round non-integer counts to the nearest integer
merged_counts <- round(merged_counts)

# Ensure all values are now integers
all(apply(merged_counts, 2, is.integer))
...
merged_counts_reordered <- merged_counts %>% select(paste(sample_data$Sample_ID))

#Analyzis parameters
file_prefix = 'FeatureCounts_L1Md_KI_vs_WT'
reference_compare_to <- "WT"
factor_of_interest <- "IFNa4KI"
comparison_variable <- "Genotype"
experiment = merged_counts_reordered
experiment
sample_data
...
ggplot(z$data,aes(x=z$data$PC1, y=z$data$PC2, )) +
  ...
  stat_ellipse(aes(fill=group), geom = 'polygon',alpha = 0.1,level = 0.9)
dev.off()
...
ggplot(z$data,aes(x=z$data$PC1, y=z$data$PC2, label = z$data$name)) +
  ...
  geom_text_repel(aes(label = sample_data$Sample_ID),max.overlaps = Inf, size = 1) +
  stat_ellipse(aes(fill=group), geom = 'polygon',alpha = 0.1,level = 0.9)
dev.off()
...
name_list <- c('Geneid', (paste0(dds_run$Genotype, "_", dds_run$Sample_ID)))
name_list
...
sig_export <- DS_norm_counts %>%
  ...
  `colnames<-`(name_list) %>%
  semi_join(sig_res) %>%
  dplyr::slice(match(sig_res$Geneid, Geneid)) 

sig_export_padj <- sig_export %>% filter(Geneid %in% sig_res_padj$Geneid)

#Manual Reordering of Columns (if necessary)
reordered_index <- c(
  grep("WT", names(sig_export), ignore.case = T),
  grep("IFNa4KI", names(sig_export), ignore.case = T))
...
sig_res_xlsx <- left_join(sig_res, count_data, by = "Geneid")
sig_res_padj_xlsx <- left_join(sig_res_padj, count_data, by = "Geneid")
check_res_xlsx <- left_join(check_res, count_data, by = "Geneid")
sig_export_xlsx <- left_join(sig_export, count_data, by = "Geneid")
sig_export_padj_xlsx <- left_join(sig_export_padj, count_data, by = "Geneid")
check_counts_xlsx <- left_join(check_counts, count_data, by = "Geneid")

#Statistics
write.xlsx(sig_res_xlsx, file = paste0('results/', file_prefix, '/DEGene_statistics_pval05.xlsx'), overwrite = T)
write.xlsx(sig_res_padj_xlsx, file = paste0('results/', file_prefix, '/DEGene_statistics_padj05.xlsx'), overwrite = T)
write.xlsx(check_res_xlsx, file = paste0('results/', file_prefix, '/Statistics.xlsx'), overwrite = T)

#Counts
write.xlsx(sig_export_xlsx, file = paste0('results/', file_prefix, '/DEGene_counts_pval05.xlsx'), overwrite = T)
write.xlsx(sig_export_padj_xlsx, file = paste0('results/', file_prefix, '/DEGene_counts_padj05.xlsx'), overwrite = T)
write.xlsx(check_counts_xlsx, file = paste0('results/', file_prefix, '/DS_counts_check.xlsx'), overwrite = T)


###### ----- HEATMAPS -----#####
data_final <- sig_export_padj %>% column_to_rownames('Geneid')
...
clusters = 2
Pval_cutoff = "padj05"
dir.create(paste0('plots/heatmaps/',file_prefix ,"_L1Md_",Pval_cutoff))
heatmap_dir <- paste0('plots/heatmaps/',file_prefix ,"_L1Md_",Pval_cutoff,"/")
...
ggsave(paste0(heatmap_dir,'ALL_DEGs.pdf'),
       phm_full,
       height = 40,
       width = 15,
       dpi = 300)
...
ggsave(paste0(heatmap_dir,'ALL_DEGs_labelled.pdf'),
       phm_full,
       width = 20,
       height = 20,
       dpi = 300)
...
#   control_4h = rowMeans(select(data_final, contains(("control_4h")))),
#   LPS_4h = rowMeans(select(data_final, contains(("LPS_4h")))),
#   control_7d = rowMeans(select(data_final, contains(("control_7d")))),
#   LPS_7d = rowMeans(select(data_final, contains(("LPS_7d"))))
# )
# 
# 
...
#TE results
Pval_cutoff <- 'padj05'
sig_res_TE <- sig_res %>% filter(padj<0.05 & baseMean > 5) %>% filter(grepl(':',Geneid))
sig_counts_TE <- sig_export %>% filter(Geneid %in% sig_res_TE$Geneid) %>% filter(grepl(':',Geneid))

goi <- sig_res_TE %>% head(40) %>% pull('Geneid')
goi_data <-  check_counts %>% filter(Geneid %in% goi)
...
families_unique <- c("LTR"   ,    "DNA"  ,      "LINE")
dev.off()
...
###### COMPARISON OF FEATURECOUNTS L1 with DEG LOCATIONS ######
#import featurecounts results:
L1_fc_result <- read.xlsx(paste0('Results/FeatureCounts_L1Md_KI_vs_WT/DEGene_statistics_padj05.xlsx'))

TEtranscript_result <-  read.xlsx(paste0('Results/TEtranscript_multi_all_samples/DEGene_statistics_padj05.xlsx'))


#Itenditify overlapü of L1 eeements with gene annotations
